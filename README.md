...
The URL is `origin` plus `pathname` plus `search`. Pathname template variables like `{{id}}` are filled from the mapping options and URL encoded. `search` is a literal object or a mapping evaluated against the current context. `method` defaults to `GET`. A `body` mapping is evaluated and sent as JSON. A `url` pointer reads a complete URL from prior output instead.
